...
PAYLOAD = bytes.fromhex("c0a801071234abcd")
...
def test_ramp_roundtrip(command, address, scope, from_tmcc_rx):
    req = RampCommandReq.for_endpoint(command, address, "192.168.1.7", 0x1234, 0xABCD, scope)
    ...
    assert command.value.num_data_bytes == 10
    ...
    packet = req.as_bytes
    assert len(packet) == 45 == req.num_bytes
    ...
    assert packet[14:42:3] == address.to_bytes(2, "big") + PAYLOAD
    assert all(packet[i : i + 2] == bytes((0xFB, 2 + (scope is CommandScope.TRAIN))) for i in range(3, 45, 3))
    ...
    for parser in (CommandReq, MultiByteReq, VariableCommandReq, RampCommandReq):
        with patch.object(VariableCommandReq, "build", wraps=VariableCommandReq.build) as build:
            ...
            build.assert_called_once_with(command, 1, list(packet[14:42:3]), scope, address_bytes=packet[1:3])
...
def test_ramp_builder_rejects_incomplete_wire_payload(payload, builder):
    with pytest.raises(ValueError, match="wire payload must contain exactly ten bytes"):
...
def test_build_defaults_and_payload_copy():
...
def test_claim_wire_encoding():
    ...
    assert req.as_bytes == bytes.fromhex(
        "f8 03 6f fb 02 0a fb 02 00 fb 02 f1 fb 02 00 fb 02 07 "
        "fb 02 c0 fb 02 a8 fb 02 01 fb 02 07 "
        "fb 02 12 fb 02 34 fb 02 ab fb 02 cd fb 02 41"
    )
...
def test_updated_logical_address_and_scope(address, scope):
    ...
    assert parsed.num_bytes == 45


def test_endpoint_and_claim_id_form_identity():
    req = RampCommandReq.for_endpoint(RAMP_COMMANDS[0], 7, "192.168.1.7", 1234, 1)
    for host, port, claim_id in (("192.168.1.8", 1234, 1), ("192.168.1.7", 1235, 1), ("192.168.1.7", 1234, 2)):
        other = RampCommandReq.for_endpoint(RAMP_COMMANDS[0], 7, host, port, claim_id)
        assert req != other
...
def test_valid_endpoints(host, port, claim_id):
    req = RampCommandReq.for_endpoint(RAMP_COMMANDS[0], 7, host, port, claim_id)
    assert bytes(req.data_bytes) == IPv4Address(host).packed + port.to_bytes(2, "big") + claim_id.to_bytes(2, "big")
    parsed = CommandReq.from_bytes(req.as_bytes)
    assert (parsed.host, parsed.port, parsed.claim_id) == (host, port, claim_id)


@pytest.mark.parametrize(
    "payload", (None, 8, "12345678", b"", PAYLOAD[:-1], PAYLOAD + b"\x00", b"\x00\x07" + PAYLOAD, [256] * 8, [-1] * 8)
)
...
def test_invalid_unicast_endpoint(host):
    with pytest.raises(ValueError):
        RampCommandReq.for_endpoint(RAMP_COMMANDS[0], 7, host, 1234, 1)
    with pytest.raises(ValueError):
...
def test_invalid_endpoint_integer(field, value):
    ...
    with pytest.raises(ValueError):
        RampCommandReq.for_endpoint(RAMP_COMMANDS[0], 7, **endpoint)
...
@pytest.mark.parametrize("size", (0, 1, 7, 8, 9, 11))
@pytest.mark.parametrize("address", (7, 3180))
def test_framed_wrong_payload_length(size, address):
    ...
    data_words = (words[4:14] + [words[4]])[:size]
...
def test_noncanonical_framing_address(address, scope):
    ...
    with patch.object(RampCommandReq, "build", wraps=RampCommandReq.build) as build:
        ...
        if address == 0:
            ...
            build.assert_called_once_with(
                RAMP_COMMANDS[0], address, list(packet[14:42:3]), scope, address_bytes=bytes(packet[1:3])
            )
