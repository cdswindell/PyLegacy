...
def _target_speed(state: EngineOrTrainState, speed: int) -> int:
    if not getattr(state, "is_ramping", False):
        return speed
    ramp = getattr(state, "ramp", None)
    if ramp is not None and getattr(ramp, "is_active", False):
        return _int_value(getattr(ramp, "target_speed", speed))
    return _int_value(getattr(state, "target_speed", speed))
...
def snapshot_from_state(state: EngineOrTrainState) -> EngineViewState:
    ...
    target_speed = _target_speed(state, speed)
