...
class AccessoryBase(GuiZeroBase, Generic[S], ABC):
    ...
    def is_power_button_on(self, widget: PowerButton) -> bool:
        if isinstance(widget, PowerButton) and hasattr(widget, "sibling"):
            print(f"is on? {widget.image} {widget.sibling.image}")
    ...
    def toggle_power_button_state(self, widget: PowerButton) -> Widget | None:
        if isinstance(widget, PowerButton) and hasattr(widget, "sibling"):
            ...
            print(f"toggle: {widget.image} {widget.sibling.image}")
        ...
        if container:
            ...
            if container.layout == "grid":
                if bool(add_spacer):
                    ...
                    spacer.tk.grid(column=0, row=container.tk.grid_size()[1], sticky="nsew")
                    spacer.tk.configure(height=40)
                    spacer.tk.grid_propagate(False)
                    self.cache(spacer)
                self.box = box = container
            else:
                if bool(add_spacer):
                    self.add_vspace(container, 40)
