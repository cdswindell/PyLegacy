...
class CatalogPanel(OverlayPanel):
    ...
    def build(self, body: Box) -> None:
        ...
        # In Landscape (compact) mode the panel is shorter, so use a slightly
        # smaller entry font that still leaves room for the close button to
        # display correctly (s_18 was too small to read comfortably).
        lb.text_size = self._gui.s_20 if self._gui.compact else self._gui.s_24
        lb.bg = "#f7f7f7"
