...
class EngineStateSource(StateSource):
    ...
    @property
    def is_active(self) -> bool:
        return (self._dir_state is not None and self._component.direction == self._dir_state) or \
            (self._stop_start_state is not None and self._component.stop_start == self._stop_start_state) or \
            (self._speed_state is not None and self._component.speed == self._speed_state)
